...
from stable_baselines3.common.utils import configure_logger, safe_mean, should_collect_more_steps
...
class OnPolicyAgent(Agent):
    ...
    def __init__(self, model: OnPolicyAlgorithm, log_interval=None, tensorboard_log=None, tb_log_name="OnPolicyAgent"):
        self.model = model
        self._last_episode_starts = [True]
        self.n_steps = 0
        self.values: th.Tensor = th.empty(0)

        self.model.set_logger(configure_logger(self.model.verbose, tensorboard_log, tb_log_name))
    ...
    def get_action(self, obs: Observation, record: bool = True) -> np.ndarray:
        ...
        if record and self.n_steps >= self.model.n_steps:
            buf.compute_returns_and_advantage(last_values=self.values, dones=np.array(self._last_episode_starts[0]))
            ...
            if self.log_interval is not None and self.iteration % self.log_interval == 0:
                ...
                self.model.logger.record("time/total_timesteps", self.num_timesteps, exclude="tensorboard")
    ...
    def learn(self, **kwargs) -> None:
        ...
        self.model._custom_logger = False  # noqa: SLF001
        self.model.learn(**kwargs)

    def save(self, path, **kwargs) -> None:
        self.model.save(path=path, **kwargs)
...
class OffPolicyAgent(Agent):
    ...
    def __init__(
        self, model: OffPolicyAlgorithm, log_interval=None, tensorboard_log=None, tb_log_name="OffPolicyAgent"
    ):
        self.model = model
    ...
    def get_action(self, obs: Observation, record: bool = True) -> np.ndarray:
        ...
        if record:
            if self.old_buffer_action is not None:
                ...
                self.model._store_transition(  # noqa: SLF001
                    buf,
                    self.old_buffer_action,
                    obs,
                    self.old_reward,
                    np.expand_dims(np.array(self.old_done), -1),
                    [self.old_info],
                )

            if self.old_done:
                self.num_collected_episodes += 1
                self.model._episode_num += 1  # noqa: SLF001
                ...
                if self.log_interval is not None and self.model._episode_num % self.log_interval == 0:  # noqa: SLF001
                    self.model.logger.record("name", self.name, exclude="tensorboard")
                    self.model.logger.record("time/episodes", self.model._episode_num, exclude="tensorboard")  # noqa: SLF001
        ...
        self.model._last_obs = obs  # noqa: SLF001

        action, buffer_action = self.model._sample_action(self.model.learning_starts, self.model.action_noise)  # noqa: SLF001
    ...
    def update(self, reward: float, done: bool) -> None:
        ...
        if should_collect_more_steps(self.model.train_freq, self.num_collected_steps, self.num_collected_episodes):
            return

        gradient_steps = self.model.gradient_steps
        if gradient_steps <= 0:
            gradient_steps = self.num_collected_steps
        if self.model.replay_buffer.pos > 0:

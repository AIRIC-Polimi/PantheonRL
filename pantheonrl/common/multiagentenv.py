...
class PlayerException(Exception):  # noqa: N818
...
class MultiAgentEnv(gym.Env, ABC):
        ...
        if partners is not None:
            ...
            for plist in partners:
                if not isinstance(plist, list) or not plist:
                    raise PlayerException("Sublist for each partner must be nonempty list")

        self.partners = partners or [[]] * (n_players - 1)
        self.partnerids = [0] * (n_players - 1)

        self._players: Tuple[int, ...] = ()
        self._obs: Tuple[Optional[np.ndarray], ...] = ()
    ...
    def resample_random(self) -> None:
        ...
        self.partnerids = [np.random.randint(len(plist)) for plist in self.partners]  # noqa: NPY002
    ...
    def step(self, action: np.ndarray) -> Tuple[Union[Observation, Any], float, bool, Dict]:
        ego_rew = 0.0
...
class TurnBasedEnv(MultiAgentEnv, ABC):
    ...
    def __init__(self, probegostart: float = 0.5, partners: Optional[List[Agent]] = None):
    ...
    def n_reset(self) -> Tuple[Tuple[int, ...], Tuple[Optional[np.ndarray], ...]]:
        self.ego_next = np.random.rand() < self.probegostart  # noqa: NPY002

...
class AdapAgent(OnPolicyAgent):
    def __init__(
        ...
        self.model.full_obs_shape = (buf.obs_shape[0] + self.model.context_size,)
    ...
    def get_action(self, obs: Observation, record: bool = True) -> np.ndarray:
        ...
        if record and self.n_steps >= self.model.n_steps:
            buf.compute_returns_and_advantage(last_values=self.values, dones=self._last_episode_starts[0])
        ...
        obs = np.concatenate((np.reshape(obs, (1, -1)), self.model.policy.get_context()), axis=1)
        if record:
    ...
    def update(self, reward: float, done: bool) -> None:
        ...
        if done and self.latent_syncer is None:
            sampled_context = SAMPLERS[self.model.context_sampler](ctx_size=self.model.context_size, num=1, torch=True)

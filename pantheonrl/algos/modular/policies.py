...
class ModularPolicy(BasePolicy):
    ...
    def __init__(
        ...
        if net_arch is None:
            net_arch = [{"pi": [64, 64], "vf": [64, 64]}] if features_extractor_class == FlattenExtractor else []
    ...
    def build_mlp_action_value_net(self, input_dim, net_arch):
        mlp_extractor = MlpExtractor(input_dim, net_arch=net_arch, activation_fn=self.activation_fn, device=self.device)
    ...
    def _build(self, lr_schedule: Callable[[float], float]) -> None:
        ...
        self.mlp_extractor, self.action_net, self.log_std, self.value_net = self.build_mlp_action_value_net(
            input_dim=self.features_dim, net_arch=self.net_arch
        )

        partner_builds = [
            self.build_mlp_action_value_net(input_dim=self.mlp_extractor.latent_dim_pi, net_arch=self.partner_net_arch)
        ...
        self.partner_mlp_extractor, self.partner_action_net, self.partner_log_std, self.partner_value_net = zip(
            *partner_builds
        )
    ...
    def evaluate_actions(
        self, obs: th.Tensor, actions: th.Tensor, partner_idx: int, action_mask: Optional[th.Tensor] = None
    ) -> Tuple[th.Tensor, th.Tensor, th.Tensor]:
        latent_pi, latent_vf, _ = self._get_latent(obs=obs)
        partner_latent_pi, partner_latent_vf = self.partner_mlp_extractor[partner_idx](latent_pi)

        distribution = self._get_action_dist_from_latent(
            latent_pi, partner_latent_pi, partner_idx=partner_idx, action_mask=action_mask
        )
        log_prob = distribution.log_prob(actions)
        values = self.value_net(latent_vf) + self.partner_value_net[partner_idx](partner_latent_vf)
